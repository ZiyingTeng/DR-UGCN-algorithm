...
class HypergraphRandomWalk:
    ...
    def build_transition_matrix(self):
        ...
        for v in range(self.n_nodes):
            ...
            if len(edges_v) == 0:
                continue  # 孤立节点
            # p(e|v) - 修改：用归一化edge_degrees作为权重
            weights = self.edge_degrees[edges_v] / (np.max(self.edge_degrees[edges_v]) + 1e-10)  # 新增：归一化权重
            weighted_edge_probs = weights / (np.sum(weights) + 1e-10)  # 加权概率，替换原有
            for e_idx, e in enumerate(edges_v):
                if issparse(self.incidence_matrix):
                    nodes_e = self.incidence_matrix[:, e].nonzero()[0]
                else:
                ...
                for u_idx, u in enumerate(nodes_e):
                    # 转移概率 p(v→u)
                    transition_matrix[v, u] += weighted_edge_probs[e_idx] * node_probs[u_idx]  # 用weighted_edge_probs
        return transition_matrix
